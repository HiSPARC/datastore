...
class HisparcSingle(tables.IsDescription):
    event_id = tables.UInt32Col(pos=0)
    timestamp = tables.Time32Col(pos=1)
    mas_ch1_low = tables.UInt16Col(dflt=-1, pos=2)
    mas_ch1_high = tables.UInt16Col(dflt=-1, pos=3)
    mas_ch2_low = tables.UInt16Col(dflt=-1, pos=4)
    mas_ch2_high = tables.UInt16Col(dflt=-1, pos=5)
    slv_ch1_low = tables.UInt16Col(dflt=-1, pos=6)
    slv_ch1_high = tables.UInt16Col(dflt=-1, pos=7)
    slv_ch2_low = tables.UInt16Col(dflt=-1, pos=8)
    slv_ch2_high = tables.UInt16Col(dflt=-1, pos=9)
